...
class API(Client):
    ...
    async def _kiwoom_before_http_reconnect(self) -> None:
        """Tear down websocket recv before HTTP session is recreated (token expiry)."""
        if self._state not in (State.CONNECTED, State.CONNECTING):
            return
        self._stop_event.set()
        with contextlib.suppress(Exception):
            await cancel(self._recv_task)
    ...
    async def _kiwoom_after_http_reconnect(self) -> None:
        """Re-open websocket on the new aiohttp session after REST token refresh."""
        if self._state not in (State.CONNECTED, State.CONNECTING):
            return
        token = self.token()
        if not token:
    ...
    async def trade(self, start: str, end: str = "") -> list[dict]:
        ...
        data = {
            ...
            "sell_tp": "0",  # 전체/매도/매수
            "dmst_stex_tp": "SOR",  # 전체/KRX/NXT/SOR
