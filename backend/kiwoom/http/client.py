        ...
        if "return_code" in body:
            match body["return_code"]:
                case 0 | 20:
                    ...
                    return res
                case 3:
                    # 3 : Token Expired — use Client.connect/Close explicitly so subclasses
                    # (e.g. kiwoom.api.API) that override connect(headers=...) still refresh.
                    print("Token expired, trying to refresh token...")
                    pre = getattr(self, "_kiwoom_before_http_reconnect", None)
                    if pre is not None:
                        await pre()
                    await Client.close(self)
                    await Client.connect(self, self._appkey, self._secretkey)
                    post = getattr(self, "_kiwoom_after_http_reconnect", None)
                    if post is not None:
                        await post()
